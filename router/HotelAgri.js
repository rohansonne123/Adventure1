...
const OrignListing=require("../models/Agri.js");
...
router.get("/:id/hotels/search", wrapAsync(async (req, res) => {
    ...
    res.render("./HotelAgri/index.ejs", { list: listings,id });
}));
//add new route
router.get("/:id/hotels/new",isloggedin,wrapAsync(async(req,res,next)=>{
    ...
    res.render("./HotelAgri/add.ejs",{id});
...
router.get("/:id/hotels/:hotelid/edit",upload,isloggedin,wrapAsync(async(req,res)=>{
    ...
    if(!listing){
        ...
        res.redirect(`/Agri/${id}/hotels`);

    }
    res.render("./HotelAgri/update.ejs",{id,listing,editImageUrl});
}))
...
router.get("/:id/hotels/:hotelid",wrapAsync(async(req,res)=>{
    ...
    if(!list){
        ...
        res.redirect(`/Agri/${id}/hotel`);

    }
        res.render("./HotelAgri/show.ejs",{id,list,Orignlist});
...
router.put("/:id/hotels/:hotelid",upload,wrapAsync(async(req,res,next)=>{
    ...
    res.redirect(`/Agri/${id}/hotels/${hotelid}`);
}))
...
router.get("/:id/hotels",wrapAsync(async(req,res)=>{
    ...
    console.log(alllist);
    //res.send();
   res.render("./HotelAgri/index.ejs",{list,id});
}) )
