...
router.get("/:id/rooms/:roomid/edit",upload,isloggedin,wrapAsync(async(req,res)=>{
    ...
    if(!listing){
        ...
        res.redirect(`/Fun/${id}/rooms`);

    }
    res.render("./RoomFun/update.ejs",{id,listing,editImageUrl});
}))
...
router.get("/:id/rooms/:roomid",wrapAsync(async(req,res)=>{
    ...
    if(!list){
        ...
        res.render("./RoomFun/show.ejs",{id,list,Orignlist});
...
router.get("/:id/rooms",wrapAsync(async(req,res)=>{
    ...
    console.log(alllist);
    //res.send();
   res.render("./RoomFun/index.ejs",{list,id});
}) )
